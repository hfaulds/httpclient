...
EOS
  ...
  def test_get_with_block_arity_2
    called = false
    res = @client.get(serverurl + 'servlet') { |r, str|
  ...
  def test_get_with_block_string_recycle
    ...
    body = []
    @client.get(serverurl + 'servlet') { |str|
      body << str
    }
  ...
  def test_post_with_sized_io_part
    ...
    @client.debug_dev = str = StringIO.new
    @client.post(serverurl + 'servlet', { :file => myio })
  ...
  def test_post_with_unknown_sized_io_part
    ...
    @client.debug_dev = str = StringIO.new
    @client.post(serverurl + 'servlet', { :file1 => myio1, :file2 => myio2 })
    assert_match(/\r\n45\r\n/, str.string)
  end
  ...
  def test_http_custom_date_header
    @client.debug_dev = (str = "")
    @client.get(serverurl + 'hello', :header => {'Date' => 'foo'})
    lines = str.split(/(?:\r?\n)+/)
